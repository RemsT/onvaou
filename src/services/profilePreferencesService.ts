...
export interface TrailPreferences {
  ...
  maxWalkMinutes: number;      // en mode « à pied » : temps de marche max vers un site (min)
}

// Vitesse de marche (km/h) pour convertir « minutes à pied » en distance à vol d'oiseau.
const WALK_KMH = 4;
/** Rayon (km) accessible à pied en `minutes` (depuis le temps de marche max du profil). */
export function walkMinutesToKm(minutes: number): number {
  return (minutes / 60) * WALK_KMH;
}
...
export const DEFAULT_PREFERENCES: TrailPreferences = {
  ...
  trailType: 'both',
  maxMinutes: null,
  maxWalkMinutes: 30, // appliqué UNIQUEMENT en mode « à pied »
};
